...
func Signup(c *gin.Context) {
	...
	if UserVerifyErr != nil {
		response.FailWithCode(50000, "注册信息不完整", c)
		return
	}
	user := &model.SysUser{Username: R.Username, Password: R.Password}
	err, userReturn := service.Register(*user)
	if err != nil {
		response.FailWithDetailed(50000, resp.SysUserResponse{User: userReturn}, fmt.Sprintf("%v", err), c)
	} else {
...
func Login2(c *gin.Context) {
	...
	UserVerifyErr := utils.Verify(R, UserVerify)
	if UserVerifyErr != nil {
		response.FailWithCode(50000, "信息不完整", c)
		return
	}
