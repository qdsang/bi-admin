...
export default {
  ...
  methods: {
    parseTime,
    async getList() {
      const { data } = await sourceList(this.projectId)
      this.list = data.list
    },
    handleNew() {
      this.formData = {}
    ...
    async handleSubmit() {
      ...
      this.formData.port = parseInt(this.formData.port, 10)
    ...
    async handleAddTable(row) {
      this.$router.push(`${this.projectPath}/source/${row.source_id}/table`).catch(_ => {})
