    ...
    <el-card body-style="padding:0;" style="margin-bottom: 20px;" class="panel-header">
      <div slot="header" style="display: flex; justify-content:space-between;">
        ...
        <span>
          ...
          <el-button v-if="this.$route.params.id !== 'create'" size="mini" type="primary" style="float: right;margin:0 10px 0 0;" @click="$router.replace(`${projectPath}/chartpanel/create`)">{{ $t('chart.createNewChart') }}</el-button>
        </span>
      </div>
    </el-card>
      ...
      handler() {
        if (this.$route.params.id !== 'create') {
          getChartById(this.$route.params.id).then(resp => {
            ...
            this.chartName = chart.chart_name || chart.name
            this.chartDesc = chart.desc
            const content = JSON.parse(chart.content) || {}
            this.dataSrc.table = content.dataSrc
            this.dataSrc.source_id = chart.source_id || content.source_id
  ...
  methods: {
    ...
    handleSave() {
      ...
      if (chartId) {
        ...
        createChart(data).then(resp => {
          // console.log(resp)
          if (resp.data.chart_id) {
            this.$router.replace(`${this.projectPath}/chartpanel/${resp.data.chart_id}`)
            this.$message({
              type: 'success',
              message: this.$t('common.saveSuccess')
            })
    ...
    handleLinkDB() {
      ...
      dashboardList().then(resp => {
        this.dashboardList = resp.data.dashboards || resp.data.list
      })
    },
    getDbByChart(id) {
      dbByChart(id).then(resp => {
        this.linkedDbIds = resp.data.list || []
      })
    },
    isDbDisbaled(db) {
    ...
    viewAllChart() {
      ...
      chartList().then(resp => {
        this.myChartList = resp.data.list
      })
    },
    switchChart(chart) {
      this.$confirm(this.$t('chart.beforeLeaveConfirm'), this.$t('common.confirm')).then(() => {
        this.$router.replace(`${this.projectPath}/chartpanel/${chart.chart_id}`)
      ...
      this.$confirm(this.$t('chart.deleteConfirm', chart.chart_name), this.$t('common.confirm')).then(() => {
        deleteChart({ chart_id: chart.chart_id }).then(() => {
          if (this.$route.params.id === chart.chart_id) {
            this.$router.push('${this.projectPath}/chartpanel/create')
            this.showMyCharts = false
          } else {
            this.viewAllChart()
          }
